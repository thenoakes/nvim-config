...
-- Helix-inspired keybindings for clean, modal editing
-- Better escape
...
-- Better window navigation (already provided by mini.basics)
-- Resize windows
...
nmap('H', '^', 'Move to beginning of line')
...
-- Quickfix navigation
nmap('<C-n>', ':cnext<CR>', 'Next quickfix')
nmap('<C-p>', ':cprev<CR>', 'Previous quickfix')
...
-- - Helix-inspired selection and editing shortcuts
...
-- Helix-inspired selection and editing shortcuts
nmap_leader('ea', 'ggVG',                                   'Select all')
nmap_leader('ew', 'viw',                                    'Select word')
nmap_leader('el', 'V',                                      'Select line')
nmap_leader('ee', 'v$',                                     'Select to end of line')
nmap_leader('eb', 'v^',                                     'Select to beginning of line')
nmap_leader('ei', 'vi(',                                    'Select inside parentheses')
nmap_leader('eI', 'va(',                                    'Select around parentheses')
nmap_leader('e[', 'vi[',                                    'Select inside brackets')
nmap_leader('e]', 'va[',                                    'Select around brackets')
nmap_leader('e{', 'vi{',                                    'Select inside braces')
nmap_leader('e}', 'va{',                                    'Select around braces')
nmap_leader("e'", "vi'",                                    'Select inside single quotes')
nmap_leader('e"', 'vi"',                                    'Select inside double quotes')
nmap_leader('e`', 'vi`',                                    'Select inside backticks')
nmap_leader('ed', ':t.<CR>',                                'Duplicate line')
nmap_leader('eD', 'dd',                                     'Delete line')
nmap_leader('ej', 'J',                                      'Join lines')
nmap_leader('es', 'i<CR><ESC>',                             'Split line')
nmap_leader('ec', 'gcc',                                    'Toggle comment')
nmap_leader('er', ':%s/\\<<C-r><C-w>\\>/<C-r><C-w>/gI<Left><Left><Left>', 'Find and replace')
nmap_leader('ey', '"+y',                                    'Yank to system clipboard')
nmap_leader('ep', '"+p',                                    'Paste from system clipboard')
nmap_leader('eP', '"+P',                                    'Paste before from system clipboard')
...
-- Helix-inspired visual mode shortcuts
xmap_leader('ec', 'gc',                                     'Toggle comment selection')
xmap_leader('er', '"hy:%s/<C-r>h/<C-r>h/gc<left><left><left>', 'Find and replace selection')
xmap_leader('ey', '"+y',                                    'Yank selection to system clipboard')
xmap_leader('ep', '"+p',                                    'Paste from system clipboard')
...
nmap_leader('lP', '<Cmd>lua local bufname = vim.api.nvim_buf_get_name(0); local root_dir = vim.fs.dirname(vim.fs.find({"tsconfig.json", "package.json", ".git"}, {path = bufname, upward = true})[1] or bufname); print("File:", bufname); print("Detected root:", root_dir); print("CWD:", vim.fn.getcwd()); print("All tsconfig files:", vim.fn.system("find " .. vim.fn.getcwd() .. " -name tsconfig.json -type f 2>/dev/null | head -10"))<CR>', 'Debug project root detection')
nmap_leader('lX', '<Cmd>lua local clients = vim.lsp.get_clients({name = "ts_ls"}); if #clients > 0 then vim.lsp.stop_client(clients[1].id); print("Stopped TypeScript LSP"); end<CR>', 'Stop TypeScript LSP')
nmap_leader('lS', '<Cmd>lua local bufname = vim.api.nvim_buf_get_name(0); local root_dir = vim.fs.dirname(vim.fs.find({"tsconfig.json"}, {path = bufname, upward = true})[1] or bufname); vim.lsp.start({name = "ts_ls", root_dir = root_dir}); print("Started TypeScript LSP with root:", root_dir)<CR>', 'Start TypeScript LSP with correct root')
nmap_leader('lQ', '<Cmd>lua local clients = vim.lsp.get_clients({name = "ts_ls"}); for _, client in ipairs(clients) do print("TypeScript LSP root:", client.config.root_dir or "Not set"); print("TypeScript LSP workspace folders:", vim.inspect(client.workspace_folders or {})); end<CR>', 'Show TypeScript LSP workspace info')
